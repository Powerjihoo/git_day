...
class _TimingStats:
    ...
    def start(self) -> None:
        ...
        self.start_cpu_time = time.process_time_ns()

    def take_split(self) -> None:
        self.end_time = time.perf_counter_ns()
        self.end_cpu_time = time.process_time_ns()
    ...
    def emit(self, note: Optional[str] = None) -> tuple:
        """
        ...
        if not self.silent:
            self.take_split()
            wall_ns = self.time
            cpu_ns = self.cpu_time
            message = f"[TIMING] Wall: {wall_ns/(10**9):6.5f}s, CPU: {cpu_ns/(10**9):6.5f}s    [{self.name}]"
            if note is not None:
                message += f" ({note})"
            self.record(message)
