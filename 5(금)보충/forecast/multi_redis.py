...
def check_redis_data(tagname):
    ...
    print("------------------------------------")
    return recent_values, forecast, alarm, timestamps,statistics  # timestamps도 반환

# 데이터 가져오기 및 시각화
def update_plot(ax, tagname):
    recent_values, forecast, alarm, timestamps,statistics = check_redis_data(tagname)
    ...
    ax.set_title(
    f'Forecast using ARIMA for Tag {tagname}, Alarm: {alarm}, '
